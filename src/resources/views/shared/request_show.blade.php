...
<div class="container--narrow attendance-detail">
        ...
        <tr class="attendance-detail__table-row is-4col">
            ...
            <td class="attendance-detail__cell">
                <span class="name-split cell-box">
                    <span class="name-split__sei">{{ $request->applicant?->sei ?? '' }}</span>
                    <span class="name-split__mei">{{ $request->applicant?->mei ?? '' }}</span>
                </span>
            </td>
    ...
    <div class="detail-actions">
        ...
        <form method="POST" action="{{ route('request.approve.store', ['attendance_correction_request_id' => $request->id]) }}">
            @csrf
